...
class Dataset:
    ...
    def _create_ordering(
        ...
        if do_stratified:
            ...
            # convert labels to sparse labels
            if labels.ndim == 1:
                pass
            elif labels.ndim == 2:
    ...
    @classmethod
    def _balance_data(
        cls, data: np.recarray, ratio: float, do_shuffle: bool = True, do_stratified: bool = True
    ) -> int:
        """Defines split edge and optionally moves data to preserve stratification or to shuffle"""
    ...
    @classmethod
    def _batch_over(
        cls, split: np.recarray, batch_size: int,
        ...
        for begin_i in range(0, len(split), batch_size):
            ...
            yield cls._ndarray_to_readonly(batch)
    ...
    def k_fold_cross_validation(
        ...
        """Generates datasets for cross validation

        Iteratively  produces  ``k_folds``  datasets  reusing  the  same  object
        for the sake of memmory efficiency.
        ...
        do_shuffle: bool
            whether to shuffle before folding
        do_stratified: bool
            whether to preserve distribution of labels accross folds

        Yields
        ------
        Dataset
            dataset of cross validation
        """
    ...
    def sub_dataset(
        ...
        """
        # TODO optimize - first filter and scale indices, than get
    ...
    def from_data(
        ...
        train_test_edge = cls._balance_data(
            ...
            do_shuffle=do_shuffle, do_stratified=do_stratified
        )

        train_val_edge = cls._balance_data(
            data[:train_test_edge], train_ratio / (train_ratio + val_ratio),
            do_shuffle=do_shuffle, do_stratified=do_stratified
        )

        return cls(
            data[:train_test_edge], data[train_test_edge:],
            train_val_edge=train_val_edge, metadata=metadata
        )

    @classmethod
    def from_splits(
        cls, train_X: np.ndarray, train_y: np.ndarray,
    ...
    def load_layout(self, path) -> None:
        ...
        for source, target in [(self._train.id, layout['train_id']), (self._test.id, layout['test_id'])]:
            if source == target:
                ...
                source_to_sorted = np.argsort(source)
                target_to_sorted = np.argsort(target)
